...
- K3 always reasons and cannot be turned off. It accepts `low`, `high` and
  `max`; the default is `max`, and a lower effort trades reasoning depth for
  latency. The K2.x `thinking` object is accepted only as an enabled
  compatibility alias.
