...
`gpt-5.5`와 `gpt-5.4`는 `none`, `low`, `medium`, `high`, `xhigh`를 지원하며 `minimal`과 `max`는 지원하지 않습니다. 최상위 `reasoning_effort`도 호환 별칭으로 허용됩니다. 두 형식을 함께 지정하면 값이 같아야 하며, 충돌 시 `400`을 반환합니다.
