---
title: 在 Claude Code 中使用 APIAny.AI
description: 在 Claude Code 中配置 APIAny.AI API Key、Base URL 和推荐模型。
---

[Claude Code](https://claude.ai/code) 可以通过环境变量或本地配置文件连接 APIAny.AI。适合在终端、编辑器和自动化开发流程中使用统一模型接口。
...
保存后重启 Claude Code。你也可以用环境变量临时测试：
...
以下截图展示 Claude Code 中的关键配置入口和保存后的使用界面。截图中的 API 地址和名称均已按 APIAny.AI 调整。

### 步骤 1

![Claude Code 配置截图 1](/images/integrations/claude-code/claude-code-cli.png)
